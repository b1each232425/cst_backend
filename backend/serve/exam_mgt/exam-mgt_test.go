...
var (
	...
	testExamPlanStudentID           = int64(99901) // 学生报名内部编号
...
func CreateTestExamData(t *testing.T) {
	...
	`, testNormalExamID, testAcademicAffair, time.Now().UnixMilli(), 1999, testDeleteExamID,
	...
	`, testOfflineExamID, testAcademicAffair, time.Now().UnixMilli(), 1999, testExamRoomInvigilatorCountBytes)
	if err != nil {
		tx.Rollback(ctx)
		t.Fatalf("插入测试考试数据失败: %v", err)
	}
	...
	// 插入考试学生记录
	nullInt := null.NewInt(0, false)
	_, err = tx.Exec(ctx, `
		INSERT INTO t_exam_student (id, student_id, exam_plan_student_id, exam_id, creator, status )
		VALUES ($1, $2, $3, $4, $5, $6)
	`, 99999, testStudent1, nullInt, testNormalExamID, testAcademicAffair, "00")
	if err != nil {
		tx.Rollback(ctx)
		t.Fatalf("插入考试学生记录失败: %v", err)
	}
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO t_exam_student (id, student_id, exam_plan_student_id, exam_id, creator, status )
		VALUES ($1, $2, $3, $4, $5, $6)
	`, 99998, testStudent1, testExamPlanStudentID, testNormalExamID2, testAcademicAffair, "00")
	if err != nil {
		tx.Rollback(ctx)
		t.Fatalf("插入考试学生记录失败: %v", err)
	}
	...
	err = tx.QueryRow(ctx, `
        ...
        VALUES ($1, $2, $3, $4, $5, $6, 1, $7, $8, 1999, '0')
        RETURNING id
	...
	err = tx.QueryRow(ctx, `
        INSERT INTO t_file (id, digest, file_name, path, belongto_path, size, count, creator, create_time, domain_id, status)
        VALUES ($1, $2, $3, $4, $5, $6, 2, $7, $8, 1999, '0')
        RETURNING id
	...
	tests := []struct {
		...
		},
		{
			...
			requestBodyGen: func() interface{} {
				...
				`, int64(99999), time.Now().UnixMilli(), int64(99999), time.Now().UnixMilli(), "14", int64(1999)).Scan(&tempExamID)
...
func TestValidateUserExamPermission(t *testing.T) {
	...
	err = conn.QueryRow(ctx, `
		...
		testExamID, currentTime, currentTime+719990, testTeacherID).Scan(&testSessionID)
	if err != nil {
	...
	tests := []struct {
		...
		forceError        string
		mockValue         string
		accessibleDomains []int64
	}{
		{
			name:              "管理员权限-应该有权限",
			userID:            testAdminID,
			examID:            testExamID,
			domain:            "assess^admin", // 管理员角色
			wantResult:        true,
			wantError:         false,
		...
		},
		{
			...
			examID:            testExamID,
			domain:            "assess^academicAffair^admin", // 教务员角色
			wantResult:        false,
			wantError:         true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := validateUserExamPermission(ctx, tt.userID, tt.examID, tt.domain, tt.accessibleDomains)
...
func TestGetExamInfo(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:        "教务员角色-正常获取考试信息",
			examID:      testNormalExamID,
			domain:      "assess.academicAffair^admin", // 教务员
			wantError:   false,
...
func TestExamList(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			method:        "GET",
			queryParams:   `q={"Action":"select","Filter":{"Name":"","Status":"","StartTime":199900000000,"EndTime":10000000000},"Page":1,"PageSize":10}`,
			userID:        testAcademicAffair,
			userRole:      2002, // 教务员角色
			expectedError: true,
...
func TestExamUser(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:          "成功查询单个用户-02",
			method:        "GET",
			queryParams:   fmt.Sprintf(`q={"data":{"Type":"02","Examinees":[{"id":%d}]}}`, testStudent1),
			userID:        testAcademicAffair,
			userRole:      2002,
			expectedError: false,
			description:   "查询单个用户的基本信息",
			checkResult: func(t *testing.T, serviceCtx *cmn.ServiceCtx) {
				...
				user := userInfos[0]
				if user.ID != testStudent1 {
					t.Errorf("用户ID不匹配: got %d, want %d", user.ID, testStudent1)
				}
				if user.Name != "测试学生" {
					t.Errorf("用户名称不匹配: got %s, want 测试学生", user.Name)
				}

			},
		},
		{
			name:          "查询单个用户-02-强制JSON解析错误",
			forceError:    "json.Unmarshal",
			method:        "GET",
			queryParams:   fmt.Sprintf(`q={"data":{"Type":"02","Examinees":[{"id":%d}]}}`, testStudent1),
			userID:        testAcademicAffair,
		...
		{
			name:          "查询单个用户-02-强制查询用户信息错误",
			forceError:    "conn.Query2",
			method:        "GET",
			queryParams:   fmt.Sprintf(`q={"data":{"Type":"02","Examinees":[{"id":%d}]}}`, testStudent1),
			userID:        testAcademicAffair,
			userRole:      2002,
		...
		{
			name:          "查询单个用户-02-notArray2",
			forceError:    "notArray2",
			method:        "GET",
			queryParams:   fmt.Sprintf(`q={"data":{"Type":"02","Examinees":[{"id":%d}]}}`, testStudent1),
			userID:        testAcademicAffair,
			userRole:      2002,
			expectedError: true,
			description:   "查询单个用户的基本信息",
			errorContains: "data.Examinees必须是数组格式",
		},
		{
			name:          "查询单个用户-02-rows.Scan2",
			forceError:    "rows.Scan2",
			method:        "GET",
			queryParams:   fmt.Sprintf(`q={"data":{"Type":"02","Examinees":[{"id":%d}]}}`, testStudent1),
			userID:        testAcademicAffair,
			userRole:      2002,
...
func TestFindConflictingExamStudents(t *testing.T) {
	if z == nil {
		cmn.ConfigureForTest()
	}
	...
	CleanTestExamData(t)
	CreateTestExamData(t)
	t.Cleanup(func() { CleanTestExamData(t) })

	conn := cmn.GetPgxConn()
	ctx := context.Background()

	examinees := []Examinee{
		{ID: 10001, ExamPlanStudentID: null.IntFrom(testExamPlanStudentID)},
	}
	...
	tests := []struct {
		...
		},
		{
			...
			ctx:           ctx,
			input:         examinees,
			currentExamID: testNormalExamID,
			wantLen:       1,
			wantError:     false,
		},
		{
			name:          "强制错误 tx.Query error",
			ctx:           context.WithValue(ctx, "findConflictingExamStudents-force-error", "tx.Query"),
			input:         examinees,
			currentExamID: testNormalExamID,
			wantLen:       0,
			wantError:     true,
		...
		{
			name:          "强制错误 rows.Scan error",
			ctx:           context.WithValue(ctx, "findConflictingExamStudents-force-error", "rows.Scan"),
			input:         examinees,
			currentExamID: testNormalExamID,
			wantLen:       0,
			wantError:     true,
			wantContains:  "强制获取学生信息错误",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tx, _ := conn.Begin(tt.ctx)
			defer tx.Rollback(tt.ctx)
			res, err := findConflictingExamStudents(tt.ctx, tx, tt.input, tt.currentExamID)
			if tt.wantError {
				if err == nil {
					t.Fatalf("期望错误但实际为nil")
				}
				if tt.wantContains != "" && !strings.Contains(err.Error(), tt.wantContains) {
					t.Fatalf("错误信息不包含预期子串，got=%v want contains=%s", err, tt.wantContains)
				}
				return
			}
			if err != nil {
			...
			if tt.input == nil || len(tt.input) == 0 {
				// 函数对空输入返回 nil
				if res != nil {
					t.Fatalf("期望 nil, got %+v", res)
				}
				return
			}
			if len(res) != tt.wantLen {
				t.Fatalf("结果长度不匹配: got %d, want %d", len(res), tt.wantLen)
			}
			if tt.wantLen > 0 {

				if !res[0].ExamPlanStudentID.Valid || res[0].ExamPlanStudentID.Int64 != testExamPlanStudentID {
					t.Fatalf("unexpected conflict: %+v", res[0])
				}
			}
		})
	}
