...
func QueryMarkingResults(ctx context.Context, cond QueryCondition) ([]cmn.TMark, error) {
	forceErr, _ := ctx.Value(ForceErrKey).(string)
	if cond.TeacherID <= 0 {
		err := errors.New("invalid params: 请求参数必须包含教师ID")
		z.Error(err.Error())
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err != nil || forceErr == "QueryMarkingResults-rows.Scan" {
			err = fmt.Errorf("unable to scan mr data: %v", err)
			z.Error(err.Error())
			return nil, err
		}
...
func QueryMarkerInfo(ctx context.Context, cond QueryCondition) (MarkerInfo, error) {
	...
	// 参数校验
	if cond.ExamSessionID == 0 && cond.PracticeID == 0 {
		err := errors.New("invalid params: either exam session id or practice id is required")
		z.Error(err.Error())
	...
	if cond.ExamSessionID > 0 {
		query = `SELECT mi.mark_teacher_id, mi.mark_count, 
                 ...
                 WHERE mi.exam_session_id = $1 AND mi.status != '04' AND mi.mark_teacher_id = $2`
		queryParams = append(queryParams, cond.ExamSessionID, cond.TeacherID)
	} else {
		query = `SELECT mi.mark_teacher_id, mi.mark_count, 
                        mi.mark_question_groups, mi.mark_examinee_ids, mi.question_ids,
                        p.correct_mode 
                 ...
                 WHERE mi.practice_id = $1 AND mi.status != '04' AND mi.mark_teacher_id = $2`
		queryParams = append(queryParams, cond.PracticeID, cond.TeacherID)
	}
...
// 查询学生的答案
func QueryStudentAnswers(ctx context.Context, answerType string, cond QueryCondition, markerInfo MarkerInfo) ([]cmn.TVStudentAnswerQuestion, error) {
	forceErr, _ := ctx.Value(ForceErrKey).(string)
	...
	if answerType == "" {
		err := errors.New("invalid answer_type")
		z.Error(err.Error())
		return nil, err
	}
	...
	case "00", "04", "10": // 试卷分配 查询单个考试考生的数据 // 重构，一次只获取一个考生的答案
	...
	case "12": // 查询单个练习学生的数据
		...
		if cond.PracticeSubmissionID <= 0 {
			err := errors.New("invalid practice_submission_id")
			z.Error(err.Error())
			return nil, err
		}
	...
	studentAnswers := make([]cmn.TVStudentAnswerQuestion, 0, 10) // 假设 10
	...
	for rows.Next() {
		...
		if err != nil || forceErr == "rows.Scan" {
			err = fmt.Errorf("unable to scan sa data: %v", err)
			z.Error(err.Error())
			return nil, err
		}
...
func QuerySubjectiveQuestions(ctx context.Context, cond QueryCondition, markerInfo MarkerInfo) ([]QuestionSet, error) {
	forceErr, _ := ctx.Value(ForceErrKey).(string)
...
func UpdateMarkerInfoState(ctx context.Context, tx *pgx.Tx, teacherID int64, ids []int64, mode string) ([]int64, error) {
	forceErr, _ := ctx.Value(ForceErrKey).(string)
	if teacherID <= 0 {
		err := errors.New("invalid teacher ID param")
		z.Error(err.Error())
		return nil, err
	}

	if ids == nil || len(ids) == 0 {
		err := errors.New("no exam_session_ids or practice_ids to update")
		z.Error(err.Error())
		return nil, err
	}
	...
	if mode == "00" {
		...
		err := errors.New("invalid update mode")
		z.Error(err.Error())
		return nil, err
	}
...
func InsertOrUpdateMarkingResults(ctx context.Context, tx pgx.Tx, markingResults []cmn.TMark) ([]int64, error) {
	...
	for _, mark := range markingResults {
		...
		if mark.QuestionID.Int64 <= 0 {
			err := errors.New("questionID is required for mark")
			z.Error(err.Error())
			return nil, err
		}
		...
		if !(validExam || validPractice) {
			err := errors.New("invalid insert params: must have either (exam session id and examinee id) both > 0 with others <= 0, or (practice id and practice submission id) both > 0 with others <= 0")
			z.Error(err.Error())
			return nil, err
		}
...
func QueryMarkedPersonAndQuestionCount(ctx context.Context, cond QueryCondition) (int64, int64, error) {
	...
	if err != nil {
		err = fmt.Errorf("query marked person error: %v", err)
		z.Error(err.Error())
		return 0, 0, nil
	}
	...
	if err != nil {
		err = fmt.Errorf("query marked question count error: %v", err)
		z.Error(err.Error())
		return 0, 0, nil
	}
...
func updateStudentAnswerScore(ctx context.Context, tx pgx.Tx, markingResults []cmn.TMark, cond QueryCondition) (targetIDs []int64, err error) {
	forceErr, _ := ctx.Value(ForceErrKey).(string)
