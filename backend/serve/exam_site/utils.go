...
// destDir 为数据保存目录
// fileName 为脚本文件名
func generateExportScriptsCentralSide(sysUser int64, destDir string, fileName string) (tableFileList []string, err error) {
...
WHERE t_exam_site.sys_user = %d`,
	...
	if err != nil || (cmn.InDebugMode && fileName == "force-create-export-script-file-err") {
	...
	for _, info := range exportInfo {
		...
		if err != nil || (cmn.InDebugMode && fileName == "force-write-export-script-file-err") {
